...
/*
1. the program fiest read the input line and get processes from the file
2. pass everything to round robin
    two linked list are created, waiting_processes and running_processes
    waiting_processes stored all the processes that is not ready
    running_processes stored all the processes that can run at the current time
3. goes into the loop
4. sort all the processes in waiting_processes based on arrival time
5. check waiting_processses and put processes that should arrive to running_processes
6. use process manager to check if processes in running_processes meet the requirement ro run
7. if can't run, put back to waiting_processes
8. run the first processes in running_processes
9. if not finish, put back to waiting_processes
10. if finish, remove
11. go back to step 4 until all processes done


process manager
1. if current process is in memory -> it can run
2. if it is not
    2.1 allocate memory, success -> it can run
    2.2 cannot allocate ->  cannot run, back to waiting_processes
*/
...
// function for calculate the output usage percentage
...
// Function to update the stat for a process for later printing
...
// read input file and create linked list of processes and stats
...
//function for counting the available page
...
// funcrtion for first-fit to calculate mem_usage
...
// function for print the pages
void find_matching_pages_as_string(struct Process** process, struct Page pages[]) {
    ...
    printf("%s\n", matched_pages_string);
    free(matched_pages_string);
}

//for virtual
void virtual_find_matching_pages_as_string(struct Process** process, struct Page pages[], int page_num_need) {
    // Allocate memory for the string to store page numbers
    char* matched_pages_string = (char*)malloc(sizeof(char) * MAX_PAGES * 10 + 3); // Assuming each page number takes at most 10 characters
    if (matched_pages_string == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
...
// put the head of a to tail of b, remove it from a
...
// copy head of a to tail of b, keep a the same
...
void remove_head(struct Process** head, struct Process** finished){
    if (*head != NULL){
        *head = (*head)->next;
    }
}
...
//allocate memory for first fit, put the entire memory to the memory space
...
// remove all the page for a process
void evict_page(struct Page pages[], int page_num_need, int time_counter, struct Process** process_history, struct Process** all_process_history){
        ...
        for(int i = 0; i < MAX_PAGES; i++){
            if(strcmp(pages[i].name, (*process_history)->name) == 0){
                if (has_printed_eviction_info == 0){
                    printf("%d,EVICTED,evicted-frames=", time_counter);
                    find_matching_pages_as_string(process_history, pages);
                    has_printed_eviction_info = 1;
                }strcpy(pages[i].name, "");
                pages[i].is_allocated = 0;
            }
        }aHead_to_bTail(process_history, all_process_history);
    }
}

// remove partes of the page for a process
    ...
    while(page_count(pages) < tool && head != NULL){
        for(int i = 0; i < MAX_PAGES && page_count(pages) < tool; i++){
            if(strcmp(pages[i].name, head->name) == 0){
                if (has_printed_eviction_info == 0){
                    printf("%d,EVICTED,evicted-frames=", time_counter);
                    virtual_find_matching_pages_as_string(&head, pages, page_num_need);
                    has_printed_eviction_info = 1;
                }strcpy(pages[i].name, "");
                pages[i].is_allocated = 0;
            }
...
// remove the page for finished process
void evict_finished_page(struct Page pages[], int page_num_need, int time_counter, struct Process** current_process){
    ...
    for(int i = 0; i < MAX_PAGES; i++){
        if(strcmp(pages[i].name, (*current_process)->name) == 0){
            if (has_printed_eviction_info == 0){
                printf("%d,EVICTED,evicted-frames=", time_counter);
                find_matching_pages_as_string(current_process, pages);
                has_printed_eviction_info = 1;
            }strcpy(pages[i].name, "");
            pages[i].is_allocated = 0;
        }
...
void page_allocate_memory(struct Page pages[], struct Process **running_process, struct Process **process_history, struct Process **all_process_history, int time_counter){
    struct Process *current_running_process = *running_process;
    if(current_running_process != NULL){
        if(page_in_page(pages, current_running_process->name) == 1){
            ...
            if(page_available < page_num_need){
                evict_page(pages, page_num_need, time_counter, process_history, all_process_history);
            }
            int counter = 0;
...
// allocate  memory for virtual, need to check if less than or more than PAGE_LIMIT(4)
...
// check if prev or next is empty, if it is, connect with current
...
void paged_process_manager(struct Page pages[], struct Process **running_processes, struct Process **process_history, struct Process **all_process_history, struct Process **waiting_processes, int quantum, char *last_process_name, int time_counter){
    int check = 0;
    while (check != 1 && *running_processes != NULL){
        struct Process *current_running_process = *running_processes;
        if (page_in_memory(pages, &current_running_process) == 1){
            ...
            page_allocate_memory(pages, &current_running_process, process_history, all_process_history, time_counter);
            check = 1;
        }
    }
}
...
int find_memory_address(struct MemoryBlock **memory, char* name){
    ...
    }return MEMORY_CAPACITY + 1; // will never happen
}

//chekc if it is in process hostory
...
void round_robin(struct Process **processes, struct Process **process_history, struct Process **all_process_history, struct Process **finished_process, struct PerformStat **statistics, struct MemoryBlock **memory, int num_process, char *memory_strategy, int quantum, int *simulation_time, struct Page pages[]){
    ...
    while (num_process_done < num_process){
        ...
        if(strcmp(memory_strategy, "first-fit") == 0){
            ...
            paged_process_manager(pages, &running_processes, process_history, all_process_history, &waiting_processes, quantum, last_process_name, time_counter);
        ...
        if (running_processes != NULL){
            ...
            if(strcmp(last_process_name, running_processes->name) != 0){
                if(strcmp(memory_strategy, "infinite") == 0){
                    ...
                    printf("%d,%s,process-name=%s,remaining-time=%d,mem-usage=%.0f%%,mem-frames=", time_counter, stateToString(running_processes->state), running_processes->name, running_processes->service_time, paged_mem_usage(pages));
                    find_matching_pages_as_string(&running_processes, pages);
            ...
            if ((running_processes->service_time - quantum) > 0){
                ...
                *simulation_time = time_counter;
                ...
                aHead_to_bTail(&running_processes, finished_process);
...
int main(int argc, char* argv[]){
    ...
    struct Process *all_process_history = NULL;
    struct Process *finished_process = NULL;
    ...
    round_robin(&processes, &process_history, &all_process_history, &finished_process, &statistics, &memory, num_processes, memory_strategy, quantum, &simulation_time, page_list);
    print_stat(&statistics, &simulation_time);
 
    free_processes(finished_process);
    free_stat(statistics);
    free_processes(all_process_history);
